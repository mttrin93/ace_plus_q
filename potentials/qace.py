...
class QACE(BACE):
    def __init__(self, n_e_atomic_prop, max_at, name_out, invert_matrix=False, *args, **kwargs):
    ...
    def save(self, prefix=None):
        ...
        #path = './saves/step_test_qm9_{}/'.format(prefix)
        #path = './saves/step_test_qm9_j_{}/'.format(prefix)
        #path = './saves/step_test_stc_dip_chg_{}/'.format(prefix)
        #path = './saves/step_test_stc_dip_{}/'.format(prefix)
    ...
    def eval_atomic_energy(self, r_ij):
        ...
        if self.invert_matrix:
            ...
            b = tf.tensor_scatter_nd_update(b, indx, tf.reshape(self.tensor_input[DATA_TOTAL_NUM_ELEC], [-1]))
