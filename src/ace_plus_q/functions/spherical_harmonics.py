    ...
    def compute_ylm_sqr(self, rhat):
        ...
        for l in range(0, self._lmax + 1):
            for m in range(-self._lmax, self._lmax + 1):
                ...
                ph_r = ph 
                ph_i = tf.zeros_like(ph, dtype=self.prec.float)#tf.constant(0, dtype=self.prec.float)
                if m < 0 and abs(m) > l:
                    sqr_ylm_r += [0 * ylm_r[0]]
                    sqr_ylm_i += [0 * ylm_i[0]]
        ...
        #return tf.reshape(ylm, [-1, self._lmax + 1, 2 * self._lmax + 1])  ##[None, nYlm, 1] -> [None, l, m]
