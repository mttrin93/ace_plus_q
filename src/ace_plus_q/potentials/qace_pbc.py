...
from src.ace_plus_q.functions import spherical_harmonics, radial_functions
from src.ace_plus_q.data.datakeys import *
from src.ace_plus_q.potentials.potential import BACE
...
class QACE_PBC(BACE):
    def __init__(self, n_e_atomic_prop, max_at, nkopints, g_ewald,
                 save_path, chi0_dict, J0_dict, sigma_dict,
                 invert_matrix=False, *args, **kwargs):
...
#            num_structures = tf.cast(self.tensor_input[DATA_NUM_OF_STRUCTURES], dtype_nat)
            ...
            b = tf.concat([chi_ext, tf.reshape(self.tensor_input[DATA_TOTAL_CHRG], [-1, 1])], axis=1)
